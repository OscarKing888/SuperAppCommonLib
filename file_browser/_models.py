...
class FileTableModel(QAbstractTableModel):
    ...
    def set_meta_for_paths(self, updates: list[tuple[str, dict | None]]) -> int:
        """Apply metadata updates and emit one signal per contiguous row range."""
        if not updates:
            return 0
        ...
        for path, meta in updates:
            ...
            if row is None:
                continue
            self._apply_meta_to_entry(self._entries[row], meta)
            changed_rows.append(row)
        if not changed_rows:
            return 0

        changed_rows = sorted(set(changed_rows))
        roles = [_DisplayRole, _SortRole, _ForegroundRole, _BackgroundRole]
        ...
        for row in changed_rows[1:]:
            ...
            self.dataChanged.emit(
                self.index(range_start, _TREE_COL_COMMENT),
...
class ThumbnailListModel(QAbstractListModel):
    ...
    def set_meta_for_paths(self, updates: list[tuple[str, dict | None]]) -> int:
        """Apply changed thumbnail metadata with contiguous range signals."""
        if not updates:
            return 0
        changed_rows: list[int] = []
        all_roles: list[int] = []
