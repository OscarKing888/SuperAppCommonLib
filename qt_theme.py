...
def scheme_from_qt_value(value) -> ColorSchemeName | None:
    if value is None:
        return None
    ...
    if application is not None:
        ...
        if callable(style_hints):
            ...
            if callable(color_scheme):
                detected = scheme_from_qt_value(color_scheme())
...
def is_theme_change_event(event) -> bool:
    """Recognize palette/theme events exposed by either PyQt binding."""
    if event is None:
        return False
    ...
    watched = (
        getattr(event_types, name, None)
        for name in ("ThemeChange", "PaletteChange", "ApplicationPaletteChange")
    )
    return any(
        item is not None and value == getattr(item, "value", item)
        for item in watched
    )
